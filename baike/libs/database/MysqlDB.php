...
class MysqlDB {
     ...
     */
    function connect() {
        $this->dbConn = mysql_connect($this->host . ':' . $this->port, $this->user, $this->pwd);
        if (!$this->dbConn) {
            throw new BaiException(__CLASS__ . ':error to connect mysql server');
        }
        if (!mysql_select_db($this->dbName)) {
            throw new BaiException(__CLASS__ . ':database not found');
        }
